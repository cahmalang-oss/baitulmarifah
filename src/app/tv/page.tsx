...
type JadwalData = {
  kajian: any[]; imam: any[];
  pengumuman: { id: string; judul?: string | null; isi: string; flyer_url?: string | null }[];
};
...
export default function TvPage() {
  ...
  useEffect(() => {
    fetchTv(); fetchJadwal(); fetchHadits();
    const tvTimer = setInterval(fetchTv, 60_000);
    const jadwalTimer = setInterval(fetchJadwal, 60_000);
    const haditsTimer = setInterval(fetchHadits, 5 * 60_000);
    return () => { clearInterval(tvTimer); clearInterval(jadwalTimer); clearInterval(haditsTimer); };
  }, [fetchTv, fetchJadwal, fetchHadits]);
