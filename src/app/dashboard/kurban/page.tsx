...
export default function KurbanSetoranPage() {
  ...
  useEffect(() => {
    loadProfile();
    fetch('/api/public/paket').then(r => r.json()).then(j => setPaketList(j.data || [])).catch(() => {});
    fetch('/api/public/settings').then(r => r.json()).then(j => { if (j.settings) setBankInfo(j.settings); }).catch(() => {});
  }, []);

  const handleDaftarPaket = async () => {
    if (!selectedPaket) { setDaftarError('Pilih salah satu paket terlebih dahulu'); return; }
    setDaftarLoading(true);
    setDaftarError('');
    try {
      const res = await fetch('/api/jamaah/daftar-paket', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ paket_id: selectedPaket }),
      });
      const data = await res.json();
      if (!res.ok) { setDaftarError(data.error || 'Gagal mendaftar paket'); setDaftarLoading(false); return; }
  ...
  return (
    <div className="p-5">
      ...
      {loadingProfile ? (
        <div className="text-center py-10 text-white/40 text-sm">Memuat data...</div>
      ) : (
        <>
          {/* ── Belum punya paket: tampilkan pilihan paket untuk daftar mandiri ── */}
          {!profile?.paket_id && (
            <>
              {daftarSukses ? (
                <div className="mb-5 bg-green-900/20 border border-green-500/30 rounded-2xl p-4">
                  <p className="text-green-300 font-bold text-sm mb-1">✓ Pendaftaran Terkirim</p>
                  <p className="text-white/60 text-xs">Permintaan bergabung Anda sedang menunggu verifikasi admin/verifikator. Anda akan bisa melapor setoran setelah disetujui.</p>
                </div>
              ) : (
                <div className="mb-5">
                  <div className="bg-[#C9A84C]/10 border border-[#C9A84C]/30 rounded-2xl p-4 mb-3">
                    <p className="text-[#C9A84C] font-bold text-sm mb-1">Belum Terdaftar Paket Qurban</p>
                    <p className="text-white/60 text-xs">Pilih salah satu program tabungan/patungan qurban di bawah untuk mendaftar. Pendaftaran akan diverifikasi oleh admin/verifikator.</p>
                  </div>

                  {daftarError && (
                    <div className="p-3 mb-3 bg-red-900/30 border border-red-500/30 text-red-300 rounded-xl text-sm">{daftarError}</div>
                  )}
                  ...
                  {paketList.length > 0 ? (
                    <div className="space-y-2 mb-3">
                      {paketList.map((p) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-3 rounded-xl border-2 transition-all ${
                            selectedPaket === p.id ? 'border-[#C9A84C] bg-[#C9A84C]/10' : 'border-white/10 bg-white/3 hover:border-white/20'
                          }`}
                        >
                          <div className="flex justify-between items-center">
                            <div>
                              <p className={`font-semibold text-sm ${selectedPaket === p.id ? 'text-[#C9A84C]' : 'text-white/80'}`}>
                                {JENIS_LABEL[p.jenis] || p.jenis} — {p.nama}
                              </p>
                              {p.deskripsi && <p className="text-xs text-white/40 mt-0.5">{p.deskripsi}</p>}
                            </div>
                            <span className={`text-sm font-bold ml-3 flex-shrink-0 ${selectedPaket === p.id ? 'text-[#C9A84C]' : 'text-white/60'}`}>
                              {fmt(p.harga_target)}
                            </span>
                          </div>
                        </button>
                      ...
                      <button
                        onClick={handleDaftarPaket}
                        disabled={daftarLoading || !selectedPaket}
                        className="w-full py-3.5 mt-2 bg-[#C9A84C] text-[#0F172A] font-bold rounded-xl hover:bg-[#D4B869] transition-colors disabled:opacity-40"
                      >
                        {daftarLoading ? 'Mendaftar...' : 'Daftar Paket Ini'}
                      </button>
                    </div>
                  ) : (
                    <p className="text-sm text-white/40 px-4 py-3 bg-white/5 rounded-xl border border-white/10">
                      Belum ada paket aktif tersedia. Hubungi admin untuk informasi paket.
                    </p>
                  )}
                </div>
              )}
          ...
          {/* ── Sudah daftar tapi menunggu verifikasi ── */}
          {profile?.paket_id && profile?.paket_status === 'pending' && (
            <div className="mb-5 bg-amber-900/20 border border-amber-500/30 rounded-2xl p-4">
              <p className="text-amber-300 font-bold text-sm mb-1">⏳ Menunggu Verifikasi</p>
              <p className="text-white/60 text-xs">
                Pendaftaran paket <strong className="text-white">{profile.paket?.nama}</strong> sedang diverifikasi admin/verifikator. Anda bisa melapor setoran setelah disetujui.
              </p>
            </div>
          )}

          {/* ── Sudah aktif: info + saldo ── */}
          {profile?.paket_id && profile?.paket_status !== 'pending' && (
          ...
          {/* Info Rekening — hanya tampil jika sudah boleh setor */}
          {profile?.paket_id && profile?.paket_status !== 'pending' && (bankInfo.no_rekening_bsi || bankInfo.info_qris) && (
          ...
          {/* Form setoran — hanya jika paket aktif (bukan pending, bukan kosong) */}
          {profile?.paket_id && profile?.paket_status !== 'pending' && (
